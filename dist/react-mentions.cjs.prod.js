...
function defaultDisplayTransform(id, display) {
  return display || id;
}
...
var emptyFn = function() {}, iterateMentionsMarkup = function(value, config, markupIteratee) {
  for (var match, textIteratee = arguments.length > 3 && void 0 !== arguments[3] ? arguments[3] : emptyFn, regex = combineRegExps(config.map(function(c) {
    ...
    var offset = captureGroupOffsets.find(function(o) {
      return !!match[o];
    }), mentionChildIndex = captureGroupOffsets.indexOf(offset), _config$mentionChildI = config[mentionChildIndex], markup = _config$mentionChildI.markup, _config$mentionChildI2 = _config$mentionChildI.displayTransform, displayTransform = void 0 === _config$mentionChildI2 ? defaultDisplayTransform : _config$mentionChildI2, idPos = offset + findPositionOfCapturingGroup(markup, "id"), displayPos = offset + findPositionOfCapturingGroup(markup, "display"), id = match[idPos], display = displayTransform(id, match[displayPos]), substr = value.substring(start, match.index);
...
}, defaultMarkup = "@[__display__](__id__)", readConfigFromChildren = function(children) {
  return React.Children.toArray(children).map(function(_ref) {
    var _ref$props = _ref.props, _ref$props$markup = _ref$props.markup, markup = void 0 === _ref$props$markup ? defaultMarkup : _ref$props$markup, regex = _ref$props.regex, displayTransform = _ref$props.displayTransform;
    return {
      markup: markup,
...
function SuggestionsOverlay(_ref) {
  ...
  var suggestionsToRender, renderSuggestion = function(result, queryInfo, index) {
    var isFocused = index === focusIndex, childIndex = queryInfo.childIndex, query = queryInfo.query, _Children$toArray$chi = React.Children.toArray(children)[childIndex].props.renderSuggestion, renderSuggestion = void 0 === _Children$toArray$chi ? null : _Children$toArray$chi;
    ...
    _defineProperty(_assertThisInitialized(_this), "setContainerElement", function(el) {
      ...
      if (null !== positionInValue) {
        ...
        React__default.Children.forEach(children, function(child, childIndex) {
          if (child) {
            var _child$props$trigger = child.props.trigger, regex = makeTriggerRegex(void 0 === _child$props$trigger ? "@" : _child$props$trigger, _this.props), match = substring.match(regex);
            if (match) {
      ...
      var id = _ref2.id, display = _ref2.display, childIndex = _ref3.childIndex, querySequenceStart = _ref3.querySequenceStart, querySequenceEnd = _ref3.querySequenceEnd, plainTextValue = _ref3.plainTextValue, value = _this.props.value || "", config = readConfigFromChildren(_this.props.children), _mentionsChild$props = React.Children.toArray(_this.props.children)[childIndex].props, _mentionsChild$props$ = _mentionsChild$props.markup, markup = void 0 === _mentionsChild$props$ ? defaultMarkup : _mentionsChild$props$, _mentionsChild$props$2 = _mentionsChild$props.displayTransform, displayTransform = void 0 === _mentionsChild$props$2 ? defaultDisplayTransform : _mentionsChild$props$2, appendSpaceOnAdd = _mentionsChild$props.appendSpaceOnAdd, onAdd = _mentionsChild$props.onAdd, start = mapPlainTextIndex(value, config, querySequenceStart, "START"), end = start + querySequenceEnd - querySequenceStart, insert = makeMentionsMarkup(markup, id, display);
